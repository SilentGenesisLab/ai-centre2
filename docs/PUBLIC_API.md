...
AI Centre 2 已监听 `0.0.0.0:8320`。2026-07-27 已完成以下公网验收：

- `/health` 返回 `200`。
- `/docs` 返回 `200`。
- `/openapi.json` 返回 `200`。
- 未带 Token 调用 TTS 返回 `401`。
- 携带 Token 的真实 TTS 返回 `200`，生成有效的 48 kHz WAV。

正式生产环境仍应增加 HTTPS；不要长期通过明文 HTTP 发送 Bearer Token。
