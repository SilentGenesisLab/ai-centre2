...
TASK_NAME = "control_plane.watermark_remove"
QUEUE_NAME = "watermark_remove"


class WatermarkJobNotFound(KeyError):
    pass


class WatermarkJobClient:
    ...
    def submit(self, request_data: dict[str, Any], priority: int) -> AsyncResult:
        ...
        self.redis.set(
            marker,
            "1",
            ex=self.settings.watermark_result_expires_seconds,
        )
        try:
    ...
    def _require_job(self, job_id: str) -> None:
        if not self.redis.exists(self._marker(job_id)):
            raise WatermarkJobNotFound(job_id)

    @staticmethod
    def _marker(job_id: str) -> str:
        return f"ai-centre2:watermark-job:{job_id}"
